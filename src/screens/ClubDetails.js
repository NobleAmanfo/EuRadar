  ...
  const renderItemView = ({ item }) => {
    return (
      <View style={{flexDirection:'row', marginVertical:10,alignItems: 'center', alignSelf:'center',borderWidth:2, borderColor: 'white', padding:10, borderRadius:10, width:'95%'}}>
        ...
        <View style = {{width:'50%'}}>
          <Text style={{ fontSize: 15, color: '#fff',  fontWeight: 'bold' }}>position: {item.additionalInfo.position === "1" ? 'Goalkeeper' : data.additionalInfo && data.additionalInfo.position === "2" ? 'Defender' : data.additionalInfo && data.additionalInfo.position === "3" ? 'Midfielder' : 'Attacker'}</Text>
  ...
  return (
    <SafeAreaView style={[styles.container1]}>
      ...
      <View style = {{width:'70%'}}>
        ...
        <Text style={[styles.lg]}>Name: {details.teamName}</Text>
        {data ? <>
          <Text style={[styles.lg]}>City: {data.city} </Text>
          <Text style={[styles.lg]}>Stadium: {data.homeVenue && data.homeVenue.name} </Text>
        </>
          : null}
      </View>
      </View>
